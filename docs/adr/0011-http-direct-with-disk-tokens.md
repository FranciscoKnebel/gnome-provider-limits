...
`Authorization: Bearer <token>` read from `auth.json`) do — a validated
approach.
...
- **Claude** — OAuth API: reads `~/.claude/.credentials.json`, calls
  ...
  `anthropic-beta: oauth-2025-04-20`. Maps `five_hour` → session,
  `seven_day` → weekly, `seven_day_sonnet`/`seven_day_opus` → model-specific.
...
transmitting tokens to third parties" — and enables all 3 providers with real
