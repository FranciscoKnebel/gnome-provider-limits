...
translated (every translatable string has a non-empty `msgstr`).
...
`{ "code": "en", "name": "English" }` pairs. `main-page.ts` reads it at
runtime to populate the prefs language dropdown — it never reads `LINGUAS`
or `.po` files at runtime (those are build-time inputs, not shipped). The
dropdown trusts the manifest and does not cross-check against compiled `.mo`
files actually present in the bundle. Generated, not hand-maintained; never
edited directly.
